// doFor development sketch by farmerkeith
...
unsigned long action2 (bool flag1, unsigned long & Time) {
  ...
  if (flag1) { // command to either start or stop
    ...
    // -----------------RUN area--------------------------------------------
    ++counter;
    repeatEvery(200, printCounter);
    // I would like to pass an argument to printCounter()
    // eg repeatEvery(200, printcounter(counter));
    // try modifying RepeatEvery so I can write
    // repeatEvery(200, printcounter, counter);
    // -------------------------------------------------------------
  }
  return counter;
} // end of void action2()

void printCounter (bool flag1, unsigned long & Time) {
// void printCounter (bool flag1, unsigned long & Time, unsigned long counter) {
  static unsigned long lastTime;
  if (flag1) {
    lastTime = Time;
    // --------------------------------------------------
